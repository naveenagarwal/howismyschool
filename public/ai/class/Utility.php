...
class Utility {
  ...
  */
	...
	*/
    ...
    /**
     * This function is used to slice save product images data into the database
     * @param array $image_slice_arr
     * @return null
     */
    public function slice_image($image_slice_arr) {
        $source_image = $image_slice_arr['image_path'].'/'.$image_slice_arr['source_image'];
        $slice_prefix = $image_slice_arr['image_path'].'/'.$image_slice_arr['slice_prefix'];
        $hor = $image_slice_arr['slice_hor'];
        $ver = $image_slice_arr['slice_ver'];
        include("ImageSlice.php");
        $slice = new ImageSlicer($source_image,$hor,$ver,$slice_prefix);
        $slice->show_image();
    }

    /**
     * This function is used to create array of product image url's
     * @param id $id
     * @param string $type
     * @return array
     * $type: ['v':'variant(default)'], ['r':'running'], ['h':'header']
     */
    public function create_product_img_url_arr($id, $type) {
        $img_url_arr = array();
        for( $i=1; $i<=16; $i++ ) {
            $img_url_arr[] = PRODUCT_IMAGE_ROOT_PATH.'/'.$id.'/product_'.$id.'_'.$type.'_'.$i.'.jpg';
        }
        return $img_url_arr;
    }
}
