...
class Authenticate {
    ...
    public function verify_user_login_attemp($user_name, $password, $remember_me, $mode = 1){
        ...
        $query = "SELECT
                ...
                if(is_array($result) and count($result)){
                 ...
                 $this->utility_connection->set_flash_message(t("Invalid user name or password")
                 ,'error');
                 return 0;
                }
    }
    ...
    public function verify_module_access($page,$is_special = FALSE){
        ...
        ABOUT_US_LINK, 'registration', WALL_URL, FRONTEND_LOGIN, SAVE_DESIGN, FRONTEND_REGISTER, LIST_DESIGNS,
        GET_DESIGN, WALL_IMAGE_URL, SAVE_IMAGE, GET_PRODUCTS_PATH);
    ...
    public function verify_email_user_name($user_name) {
        ...
        $query = 'SELECT user_table.user_name, user_details.email_id,
                ...
                AND (user_table.user_name="' . $user_name . '"
                OR user_details.email_id="'.$user_name.'")';
        $result = $this->db_connection->sql_single_query($query);
        if(empty($result)) {
            return false;
        } else {
