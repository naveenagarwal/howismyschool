...
$image_physical_path= PRODUCT_IMAGE_PHYSICAL_PATH.'/'.$pk_field;
$image_url_path     = PRODUCT_IMAGE_ROOT_PATH.'/'.$pk_field;
...
if(!empty($_POST['submit'])){
    ...
    if(!$error_flag){
        ...
        if( $product_obj->check_duplicate_column('product_number',$_POST['product_number'],$pk_field) ) {
            $error_flag = true;
        ...
        if( !$error_flag ) {
            if ( !empty($_FILES['image_path']['name']) ) {
                ...
                $logo_image_name = $_POST['old_logo_image'];
            }
            ...
            if( !empty($_FILES['variant_image']['name']) ) {
                ...
                $variant_image_name = $_POST['old_variant_image'];
            }
        }
        ...
        if( !$error_flag ){
            ...
            if( $product_obj->save_product_data($data) ) {
                ...
                if( $product_obj->save_product_references_data($data) ) {
                    ...
                    $image_name = 'product_'.$pk_field.'_logo.'.$file_parts['extension'];
                    $image_data_arr = array(
                        'extension'  => $file_parts['extension'],
                        'image_path' => $image_physical_path,
                        'temp_file'  => $temp_file,
                        'image_name' => $image_name,
                        'image_width'=> PRODUCT_IMAGE_WIDTH,
                        'thumbnail'  => 1,
                        'old_image'  => $logo_image_name,
                        'type'       => 'edit'
                    );
                    ...
                    $image_name = 'product_'.$pk_field.'_variant.'.$file_parts_variant['extension'];
                    $image_data_arr = array(
                        'extension'  => $file_parts_variant['extension'],
                        'image_path' => $image_physical_path,
                        'temp_file'  => $temp_file_variant,
                        'image_name' => $image_name,
                        'old_image'  => $variant_image_name,
                        'type'       => 'edit'
                    );
                    $utility_obj->upload_image($image_data_arr);
                    $variant_image_name = $image_name;
                    ...
                    if($file_parts_variant['extension']!=''){
                        $image_slice_arr = array(
                            ...
                            'slice_prefix'  => 'product_'.$pk_field,
                            'slice_hor'  => '16',
                            'slice_ver'  => '1'
                        );
                    ...
                    if( $product_obj->save_product_images_data($data) ) {
                        $data                                   = array();
...
if( $pk_field == '' ) {
    ...
    $logo_image_name    = $result_set['logo_image'];
    $variant_image_name = $result_set['variant_image'];
