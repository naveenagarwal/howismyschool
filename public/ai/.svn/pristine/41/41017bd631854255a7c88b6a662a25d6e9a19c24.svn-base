...
$edit_path       = 'texture/TextureEdit';
$delete_path     = 'texture/TextureDelete';
$add_path        = 'texture/TextureAdd';
...
if(isset($_REQUEST['data-list']) && $_REQUEST['data-list'] == '1'){
	...
	//load model class
	include_once('./model/texture/Texture.php');
	$texture_obj = new Texture($db_obj);

	$total_records = $texture_obj->get_total_textures($search);
	$result_set    = $texture_obj->get_texture_list($sort_order,$limit,$search);
...
$record_url = URL."texture/TextureList&ajax=1&data-list=1";
$delete_url = URL."texture/TextureDelete&ajax=1";
...
require_once('./view/texture/TextureList.php');
?>