...
$edit_path       = 'size/SizeEdit';
$delete_path     = 'size/SizeDelete';
$add_path        = 'size/SizeAdd';
...
if(isset($_REQUEST['data-list']) && $_REQUEST['data-list'] == '1'){
	...
	//load model class
	include_once('./model/size/Size.php');
	$size_obj = new Size($db_obj);

	$total_records = $size_obj->get_total_sizes($search);
	$result_set    = $size_obj->get_size_list($sort_order,$limit,$search);
...
$record_url = URL."size/SizeList&ajax=1&data-list=1";
$delete_url = URL."size/SizeDelete&ajax=1";
...
require_once('./view/size/SizeList.php');
?>