...
class Manufacturer {
    ...
    public $manufacturer_type = array();
	...
	public function __construct($db_obj) {
        ...
        $this->manufacturer_type = array(
                                        BRICK_MANUFACTURER=>'Brick Manufacturer',
                                        MORTAR_MANUFACTURER=>'Mortar Manufacturer'
                                    );
	}
    ...
    public function get_manufacturer_list($order_by_clause = ' id ASC',
        ...
        IF(type = 2, '".t("Mortar")."', '".t("Brick")."') as type,
    ...
    public function save_manufacturer_data($data_array) {
		if(isset($data_array['pk'])){
            ...
            type="'.$this->db_connection
                ->mysql_data_encode($data_array['type']).'",
		  ...
		  is_active,address,contact_number,email_id,type,created_by,created_at)
          ...
          .$this->db_connection->mysql_data_encode($data_array['type']).'",'
	...
	/**
     * This function is used to get manufacturer list by type
	 * @param int $type
	 * @return object
     */
    public function get_manufacturer_by_type($type) {
		$query = "SELECT id, company FROM ".$this->table_manufacturer;

		if($type!=''){
			$query .= ' WHERE is_active = 1 AND type='.$type;
		}
