...
if(!empty($_POST['submit'])){
    ...
    if(!$error_flag){
        ...
        if($color_obj->check_duplicate_column('name',$_POST['name'],
        $pk_field)){
            $error_flag = true;
            $utility_obj->set_flash_message(t('This color name already exists'),
            'error');
        }

		$image_name='';
		if(!$error_flag){
			$target_path = COLOR_IMAGE_PHYSICAL_PATH;
			if (!empty($_FILES['image_path']['name'])) {
				$temp_file = $_FILES['image_path']['tmp_name'];
				$temp_name = $_FILES['image_path']['name'];
				$file_parts = pathinfo($_FILES['image_path']['name']);
				$image_name = 'color'.time().'.'.$file_parts['extension'];
				...
				if($_FILES['image_path']['name'] !='' 
				 && $_FILES['image_path']['tmp_name'] == ''){
					...
					if(!empty($old_image) && file_exists(COLOR_IMAGE_PHYSICAL_PATH.'/'.$old_image)){
						@unlink(COLOR_IMAGE_PHYSICAL_PATH.'/'.$old_image);//delete previous logo
					}
					$utility_obj->make_thumbnail_image($temp_file,$target_file,COLOR_IMAGE_WIDTH);
				} else {
        ...
        if(!$error_flag){
            ...
            if($color_obj->save_color_data($data)){
                $success_flag = true;
...
if($pk_field ==''){
    ...
    //fetch records
    $result_set  = $color_obj->load_color_details($pk_field);
...
require_once('./view/color/ColorEdit.php');
?>