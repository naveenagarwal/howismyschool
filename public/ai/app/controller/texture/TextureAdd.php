...
if(!empty($_POST['submit'])){
    ...
    if(!$error_flag){
        ...
        if($texture_obj->check_duplicate_column('name',$_POST['name'])){
            $error_flag = true;
            $utility_obj->set_flash_message(t('This texture name already exists'),
            'error');
        }

		$image_name='';
		if(!$error_flag){
			$target_path = TEXTURE_IMAGE_PHYSICAL_PATH;
			if (!empty($_FILES['image_path']['name'])) {
				$temp_file = $_FILES['image_path']['tmp_name'];
				$temp_name = $_FILES['image_path']['name'];
				$file_parts = pathinfo($_FILES['image_path']['name']);
				$image_name = 'texture'.time().'.'.$file_parts['extension'];
				...
				if($_FILES['image_path']['name'] !=''
				 && $_FILES['image_path']['tmp_name'] == ''){
					...
					$utility_obj->make_thumbnail_image($temp_file,$target_file,TEXTURE_IMAGE_WIDTH);
				} else {
...
$noRecords = false;
require_once('./view/texture/TextureAdd.php');
?>