    ...
    if(!$error_flag){
		...
		if(!$error_flag){
			...
			if (!empty($_FILES['image_path']['name'])) {
				...
				$image_name = 'manufacturer'.time().'.'.$file_parts['extension'];
				...
				if($_FILES['image_path']['name'] !=''
				 && $_FILES['image_path']['tmp_name'] == ''){
					...
					if(!empty($old_image) && file_exists(MANUFACTURER_IMAGE_PHYSICAL_PATH.'/'.$old_image)){
						@unlink(MANUFACTURER_IMAGE_PHYSICAL_PATH.'/'.$old_image);//delete previous logo
					}
        ...
        if(!$error_flag){
            ...
            if($manufacturer_obj->save_manufacturer_data($data)){
                $success_flag = true;
...
$types_array = $manufacturer_obj->manufacturer_type;
...
if($pk_field ==''){
    ...
    //fetch records
    $result_set  = $manufacturer_obj->load_manufacturer_details($pk_field);
...
require_once('./view/manufacturer/ManufacturerEdit.php');
?>